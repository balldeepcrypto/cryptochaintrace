...
- **XLM via Stellar Horizon (3-node failover)**: `horizon.stellar.org` (primary) → `horizon-eu.stellar.org` → `stellar.publicnode.org`. All share the same REST API; 400/404 returns `{_empty: true}` gracefully. Transaction list uses `/accounts/{address}/transactions?limit=200&order=desc` (one row per transaction envelope — `from=source_account`, `to=null`, `value="0.000000"`, `direction` based on source_account === address). Cursor = `paging_token` of last record.
