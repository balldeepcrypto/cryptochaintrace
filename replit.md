...
- **XDC via RPC + BlocksScan**: Balance and tx count via XDC JSON-RPC (`rpc.xinfin.network`, `erpc.xinfin.network`, `rpc.ankr.com/xdc`). Transaction history via `https://xdc.blocksscan.io/api` (etherscan-compat format: `?module=account&action=txlist&address=...&page=...&offset=...&sort=desc`). Falls back to CoinStats (`connectionId=xdce-crowd-sale`) if blocksscan is rate-limited and `COINSTATS_API_KEY` is set. XDC addresses can be `0x`-prefixed or `xdc`-prefixed — `normalizeXdcAddress()` converts xdc→0x. NOT in `COINSTATS_CHAINS` (removed). Both CoinStats fallback calls are wrapped in try/catch to prevent error propagation.
